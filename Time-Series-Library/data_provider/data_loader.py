...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        # train test val:[0,12,16,20]->ratio【0.6,0.2,0.2】
...
class UEAloader(Dataset):
    ...
    def __init__(self, root_path, file_list=None, limit_size=None, flag=None):
        # file list是None ,limit_size也是None
        self.root_path = root_path
    ...
    def load_all(self, root_path, file_list=None, flag=None):
        ...
        if flag is not None:
            # 搜索里面带有flag的，注意这里的flag就是大写的train，刚好是文件命里面会包含的那种
            data_paths = list(filter(lambda x: re.search(flag, x), data_paths))
        input_paths = [p for p in data_paths if os.path.isfile(p) and p.endswith('.ts')]
        if len(input_paths) == 0:
            raise Exception("No .ts files found using pattern: '{}'".format(flag))
        # 实际选取的是 './dataset/JapaneseVowels\\JapaneseVowelsEq_TRAIN.ts'
        all_df, labels_df = self.load_single(input_paths[0])  # a single file contains dataset

        return all_df, labels_df

    def load_single(self, filepath):
        # 这里的排列就很奇怪，在df里面每一行代表一个一个训练数据，一个dim就是字面意义上的一个维度的信息，在一行和一个dim里面，也就是df里面的一格代表的是一个维度上一个序列的数据，他被直接压缩在了里面
        #
        ...
        # [len,dim]获取了每个单元格，与第一列中所有元素作为对比，确认同一个行里面的元素都是一样长的
        horiz_diffs = np.abs(lengths - np.expand_dims(lengths[:, 0], -1))

        if np.sum(horiz_diffs) > 0:  # if any row (sample) has varying length across dimensions
            # 这里这一步采样也感觉不明不白，只是按照比例进行的采样
            df = df.applymap(subsample)

        # 下面是获取了第一行中的元素，来确保同一个dim中元素长度相等，
        lengths = df.applymap(lambda x: len(x)).values
        vert_diffs = np.abs(lengths - np.expand_dims(lengths[0, :], 0))
        if np.sum(vert_diffs) > 0:  # if any column (dimension) has varying length across samples
            # 上一步已经确定了同一个行里面的元素都是一样长了，这样只需要找出来最长的那一个行即可即可
        ...
        # df [single_len*num_sample,dim] for japanese:[270*25,12]
        return df, labels_df

    def instance_norm(self, case):
        # 这个是用来做特殊处理的，来判断一个字符串中是否有某一段字符出现
