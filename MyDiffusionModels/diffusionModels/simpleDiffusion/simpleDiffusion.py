...
class DiffusionModel(nn.Module):
    def __init__(self,
        ...
        # alpha [time_step]
        # betas [time_step]
        # alphas_cumprod [time_step]
        # pad，中间那个代表的是左右分别填充几个值，value代表的是填充值
        # alphas_cumprod_prev [time_step],相当于从上一步的tensor去掉了最右侧的值，并在左侧填充了个1，右侧不进行填充
        # sqrt_recip_alphas [time_step]
        # 全都是time_step
    ...
    def compute_loss(self, x_start, t, noise=None, loss_type="l1"):
        # x_start就是原始图像，train过程中noise是None
        if noise is None:
            noise = torch.randn_like(x_start)
    ...
    def p_sample_loop(self, shape):
        ...
        # 这里的b是sample batch_size
        b = shape[0]
        # start from pure noise (for each example in the batch)
        # 之后是从随机噪声开始的，然后结果是输出为一个dataset里面出现过的图片
        img = torch.randn(shape, device=device)
        imgs = []
        # full这里生成了[batch_size],里面数值全是i
