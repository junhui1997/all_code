...
gestures_MISAW = ['Idle', 'Needle holding', 'Suture making',
                  'Suture handling', '1 knot', '2 knot', '3 knot']
...
def get_cross_val_splits():
    ...
    # 这里相当于是把三个train test都给写了进来
    # 其中每一个相当于一个split，每个split里面仅包含有train和test就是按照下文中split里面所写的
...
'''
def plot_barcode3(gt=None, pred=None, probability=None, visited_pos=None,
                 show=True, save_file=None):
    from config import gesture_class_num
    ...
    if probability is not None:
        frame_id = [i/10 for i in range(len(gt))]
	    plt.clf()
	    plt.plot(frame_id, pred,color="blue")
	    plt.xlabel("Frame")
	    plt.ylabel("probability")
	    plt.title("Video-"+index)
	    plt.savefig(os.path.join(os.path.split(save_file)[0]+"probability.png")

	    plt.clf()
	    plt.plot(frame_id, gt_phase_array,color="red",linewidth=2)
	    plt.plot(frame_id, pre_g_phase_array,color="blue",linewidth=1)
	    plt.xlabel("Frame")
	    plt.ylabel("category")
	    plt.title("Video-"+index)
	    plt.savefig(os.path.join(results_folder_path, 'video' + index +'_category.png'))
