        ...
        for test_loader in test_loaders:
            ...
            for _, batch in enumerate(test_loader):
                ...
                if args.return_feature:
                    out, feature = net(img_data, return_feature=True)
            ...
            if args.return_feature:
                ...
                save_mat = {"A":hidden_features, "Y":labels_all.reshape(-1,1)}

                root_name = "./features/"+args.task+"/Split_"+str(split+1)
                if not os.path.exists(root_name):
                    os.makedirs(root_name)
                scio.savemat(root_name+"/"+video_id+'.avi.mat', save_mat)
